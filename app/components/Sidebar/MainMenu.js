...
function MainMenu(props) {
  ...
  const getMenus = menuArray => menuArray.map((item, index) => {
    if (localStorage.getItem('esCoach') === "1" && item.couch === '1') {
      return level(item, index, classes, openSubMenu, open, props);
      //return level1(item, index, classes, openSubMenu, open);
      //return level2(item, index, classes, openSubMenu, open);
    } else if (localStorage.getItem('esCoach') === "0" && item.couch === '0') {
      return level(item, index, classes, openSubMenu, open, props);
      //return level1(item, index, classes, openSubMenu, open);
      //return level2(item, index, classes, openSubMenu, open);
    } else {}
...
function level(item, index, classes, openSubMenu, open, props) {
  const handleClick = () => {
    const { toggleDrawerOpen, loadTransition } = props;
    toggleDrawerOpen();
    loadTransition(false);
...
function level1(item, index, classes, openSubMenu, open) {
  if (item.title) {
    return (
      <ListSubheader
        disableSticky
...
function level2(item, index, classes, openSubMenu, open) {
  return (
    <ListItem
      key={index.toString()}
      button
